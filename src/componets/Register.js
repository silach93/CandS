...
export default function Login() {
    ...
    const [repasswordError, resetPasswordError] = useState("");
    ...
    const isValid = () => {
        ...
        if (password !== repassword) {
            resetPasswordError("Пароль должны совпадать.");
            result = false;
        }
        return result;
    };
    ...
    return <>
        <form onSubmit={handleSubmit}>
            ...
            <div>
                <label>Повтор пароля:
                    <input type="password" value={repassword} onChange={e => resetPassword(e.target.value)}/>
                </label>
                {repasswordError && <div className={styles.error}>
                    {repasswordError}
                </div>}
            </div>
